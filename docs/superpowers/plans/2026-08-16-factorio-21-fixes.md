...
public class ParseBlueprintTest : BaseTest
{
    ...
    /// </summary>
    private const string MirroredPumpjacks = "REPLACE_ME";
...
Generate the blueprint string with the probe-mod technique in CLAUDE.md, using exactly these entities:
...
Paste the exported string in place of `REPLACE_ME`.
...
- [ ] **Step 1: Generate the fixture from the game**

Use the probe-mod technique in CLAUDE.md with exactly these entities:
...
```csharp
    ...
    /// </summary>
    private const string FourCardinalPumpjacks = "REPLACE_ME";
...
**Placeholder scan.** Two `REPLACE_ME` constants remain by design, in Tasks 4 and 6. They cannot be filled in ahead of time because the blueprint strings must come out of a running Factorio. Both carry the exact `set_blueprint_entities` call that generates them, so the step is mechanical.
