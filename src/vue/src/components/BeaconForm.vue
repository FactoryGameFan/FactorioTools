<template>
  <fieldset class="border p-3 h-100">
    ...
    <ModuleSelect
      ...
      :defaultValue="beaconModule"
      :defaultIsCustom="beaconModuleIsCustom"
      v-model="beaconModule"
      v-model:isCustom="beaconModuleIsCustom"
      :showProductivityModules="isCustom"
    />
    <div class="row" v-show="showAdvancedOptions && addBeacons">
      <div class="col-lg-3 mt-3">
        <label for="beacon-entity-name" class="form-label"
          >Entity internal name (<a href="#" @click.prevent="reset">reset</a>)</label
        >
        <input
          type="text"
          class="form-control"
          id="beacon-entity-name"
          v-model="beaconEntityName"
          autocomplete="off"
        />
      </div>
      <div class="col-lg-3 mt-3">
        ...
        <input
          ...
          max="99"
          class="form-control"
          id="beacon-module-slots"
          v-model="beaconModuleSlots"
          required
        />
      </div>
      <div class="col-lg-3 mt-3">
        <p class="form-label">
          Entity size (<label for="beacon-width">width</label> x
          <label for="beacon-height">height</label>)
        </p>
      ...
      <div class="col-lg-3 mt-3">
        <p class="form-label">
          Supply area (<label for="beacon-supply-width">width</label> x
          <label for="beacon-supply-height">height</label>)
        </p>
        <div class="input-group">
          <input
            type="number"
            ...
            max="99"
            class="form-control"
            id="beacon-supply-width"
            v-model="beaconSupplyWidth"
            required
          />
          <span class="input-group-text">x</span>
          <input
            ...
            max="99"
            class="form-control"
            id="beacon-supply-height"
            v-model="beaconSupplyHeight"
            required
          />
        </div>
      </div>
    ...
    <div class="form-check mt-3" v-if="addBeacons && showAdvancedOptions">
      <input
        ...
        id="overlap-beacons"
        v-model="overlapBeacons"
      />
      <label class="form-check-label" for="overlap-beacons">
        Overlap beacon effects (e.g. uncheck to avoid
        <a href="https://spaceexploration.miraheze.org/wiki/Beacon_Overload">beacon overload</a> in
        Space Exploration)
      </label>
    </div>
  </fieldset>
